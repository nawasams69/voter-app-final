import streamlit as st
from supabase import create_client

# --- পেজ সেটআপ ---
st.set_page_config(page_title="ভোটার অনুসন্ধান", page_icon="🗳️", layout="centered")

# --- CSS ডিজাইন (ক্লিন এবং মডার্ন) ---
st.markdown("""
    <style>
    div.stButton > button {
        width: 100%;
        background-color: #f0f2f6;
        border: 1px solid #d0d0d0;
        color: black;
        border-radius: 8px;
        font-weight: bold;
    }
    div.stButton > button:hover {
        border-color: #0d6efd;
        color: #0d6efd;
    }
    .voter-card {
        background-color: white;
        padding: 15px;
        border-radius: 10px;
        box-shadow: 0 2px 6px rgba(0,0,0,0.1);
        margin-bottom: 10px;
        border-left: 5px solid #0d6efd;
    }
    .voter-name { font-size: 1.1rem; font-weight: bold; color: #212529; }
    .voter-sub { font-size: 0.9rem; color: #6c757d; }
    
    /* পপ-আপ স্টাইল কন্টেইনার */
    .detail-box {
        background-color: #e7f1ff;
        padding: 20px;
        border-radius: 10px;
        border: 1px solid #0d6efd;
        margin-bottom: 20px;
    }
    </style>
""", unsafe_allow_html=True)

# --- সেশন স্টেট (State Management) ---
# এটিই সেই ম্যাজিক যা ক্লিক করার পর ডাটা ধরে রাখবে
if 'selected_voter' not in st.session_state:
    st.session_state.selected_voter = None

def view_details(voter):
    st.session_state.selected_voter = voter

def close_details():
    st.session_state.selected_voter = None

# --- বাংলা হেল্পার ---
def get_bangla_number(n):
    return str(n).translate(str.maketrans("0123456789", "০১২৩৪৫৬৭৮৯"))

DAYS = ["দিন"] + [get_bangla_number(i) for i in range(1, 32)]
MONTHS = ["মাস", "জানুয়ারি", "ফেব্রুয়ারি", "মার্চ", "এপ্রিল", "মে", "জুন", "জুলাই", "আগস্ট", "সেপ্টেম্বর", "অক্টোবর", "নভেম্বর", "ডিসেম্বর"]
YEARS = ["বছর"] + [get_bangla_number(i) for i in range(2008, 1900, -1)]

MONTH_MAP_BN = {
    "জানুয়ারি": "০১", "ফেব্রুয়ারি": "০২", "মার্চ": "০৩", "এপ্রিল": "০৪", "মে": "০৫", "জুন": "০৬",
    "জুলাই": "০৭", "আগস্ট": "০৮", "সেপ্টেম্বর": "০৯", "অক্টোবর": "১০", "নভেম্বর": "১১", "ডিসেম্বর": "১২"
}

# --- লোকেশন ডাটা ---
LOCATION_DATA = {
    "চট্টগ্রাম সিটি কর্পোরেশন": {
        "ওয়ার্ড নং-০৩ (বায়জিদ বোস্তামী)": ["ওয়াজেদিয়া", "নাথপুর", "ভোলাগঞ্জ", "হরিপুর", "নেজামেহামজা", "মিয়ানগর", "ঘাটঘর", "পূর্ব শহীদনগর", "পশ্চিম শহীদনগর", "ছালেহনগর", "আমিননগর", "হাজীরপুল", "হামিদপুর", "খোন্দকারাবাদ", "হাজীপাড়া", "উত্তর মুরাদনগর", "দক্ষিণ মুরাদনগর", "ফকিরাবাদ", "ভক্তপুর", "পাঠানপুর", "ককসুনগর", "সোনাপুর", "করিমগঞ্জ", "জানআলী নগর", "নজীরাবাদ", "মীরপুর", "কামরাবাদ", "সামাদপুর", "জাহানপুর"],
        "ওয়ার্ড নং-০৪ (চাঁদগাও)": ["অদুর পাড়া", "উত্তর চান্দগাঁও বেপারী পাড়া", "উত্তর চান্দগাঁও চৌধুরী পাড়া", "উত্তর চান্দগাঁও চৌধুরী পাড়া শিল্প এলাকা", "উত্তর চান্দগাঁও বড়ুয়া পাড়া", "ওয়াপদা কলোনী", "কোদালকাটা", "চান্দগাঁও আবাসিক এলাকা", "চান্দগাঁও আবাসিক এলাকা ব্লক-এ", "ডালি পাড়া", "খলিফাপাড়া ও দক্ষিণ চান্দগাঁও", "দক্ষিণ পশ্চিম চান্দগাঁও শহীদ পাড়া", "নাথ পাড়া", "নূরুল হক কন্ট্রাকটর বাড়ী", "পাঠানিয়া গোদা", "ফরিদের পাড়া", "মাইজ পাড়া", "মৌলভী পাড়া", "শমসের পাড়া", "হামিদ চর", "হায়দার আলী নজীর বাড়ী ও জালাল খান", "উত্তর চান্দগাঁও হিন্দু পাড়া"],
        "ওয়ার্ড নং-০৫ (চাঁদগাও)": ["চর মোহরা সি ব্লক", "চর রাঙ্গামাটিয়া", "মোহরা এ ব্লক", "মোহরা বি ব্লক", "মোহরা সি জে এম সি এল কলোনী"],
        "ওয়ার্ড নং-০৬ (চাঁদগাও)": ["কে,বি,আমান আলী রোড ব্লক-১", "কে,বি,আমান আলী রোড ব্লক-২", "কে,বি,আমান আলী রোড ব্লক-৩", "কে,বি,আমান আলী রোড ব্লক-৪", "পূর্ব ষোলশহর ব্লক-এ", "পূর্ব ষোলশহর ব্লক-বি", "পূর্ব ষোলশহর ব্লক-সি"],
        "ওয়ার্ড নং-০৭ (পাঁচলাইশ)": ["আমিন অফিসার্স কলোনী", "আমিন লেবার কলোনী", "আতুরার ডিপো", "আলী নগর", "খতিবের হাট", "নাজির পাড়া", "মোমেন বাগ", "বার্মা কলোনী", "বিবির হাট", "মোহাম্মদ নগর", "মোহাম্মদ পুর", "রৌফাবাদ কলোনী", "শান্তি নগর", "হামজার খাঁ লেইন", "হামজার দিঘীর পাড়", "হামজার বাগ", "হামজার বাগ কলোনী", "হিলভিউ", "বন গবেষণাগার"]
    },
    "বোয়ালখালী পৌরসভা": {
        "ওয়ার্ড নং-০১": ["পশ্চিম খিতাবচর (অংশ)", "কধুরখীল (অংশ)"],
        "ওয়ার্ড নং-০২": ["কধুরখীল (পাঠান পাড়া অংশ)", "পূর্ব গোমদন্ডী (২ নং ওয়ার্ড অংশ)"],
        "ওয়ার্ড নং-০৩": ["পূর্ব গোমদন্ডী (৩ নং ওয়ার্ড অংশ-১)", "পূর্ব গোমদন্ডী (৩ নং ওয়ার্ড অংশ-২)"],
        "ওয়ার্ড নং-০৪": ["পূর্ব গোমদন্ডী (৪ নং ওয়ার্ড অংশ-১)", "পূর্ব গোমদন্ডী (৪ নং ওয়ার্ড অংশ-২)"],
        "ওয়ার্ড নং-০৫": ["পূর্ব গোমদন্ডী (৫ নং ওয়ার্ড অংশ-১)", "পূর্ব গোমদন্ডী (৫ নং ওয়ার্ড অংশ-২)"],
        "ওয়ার্ড নং-০৬": ["পূর্ব গোমদন্ডী (৬ নং ওয়ার্ড অংশ-১)", "পূর্ব গোমদন্ডী (৬ নং ওয়ার্ড অংশ-২)"],
        "ওয়ার্ড নং-০৭": ["পশ্চিম গোমদন্ডী (৭ নং ওয়ার্ড অংশ-১)", "পশ্চিম গোমদন্ডী (৭ নং ওয়ার্ড অংশ-২)"],
        "ওয়ার্ড নং-০৮": ["পশ্চিম গোমদন্ডী (৮ নং ওয়ার্ড অংশ-১)", "পশ্চিম গোমদন্ডী (৮ নং ওয়ার্ড অংশ-২)"],
        "ওয়ার্ড নং-০৯": ["পশ্চিম গোমদন্ডী (৯ নং ওয়ার্ড অংশ-১)", "পশ্চিম গোমদন্ডী (৯ নং ওয়ার্ড অংশ-২)"]
    },
    "ইউনিয়ন পরিষদ (বোয়ালখালী)": {
        "কধুরখীল ইউনিয়ন": ["কধুরখীল (অংশ) ১ নং ওয়ার্ড", "কধুরখীল (অংশ) ২ নং ওয়ার্ড", "কধুরখীল (অংশ) ৩ নং ওয়ার্ড", "কধুরখীল (অংশ) ৪ নং ওয়ার্ড", "কধুরখীল (অংশ) ৫ নং ওয়ার্ড", "কধুরখীল (অংশ) ৬ নং ওয়ার্ড", "কধুরখীল (অংশ) ৭ নং ওয়ার্ড", "কধুরখীল (অংশ) ৮ নং ওয়ার্ড", "কধুরখীল (অংশ) ৯ নং ওয়ার্ড"],
        "পশ্চিম গোমদন্ডী ইউনিয়ন": ["চরখিজিরপুর", "চরখিজিরপুর (পূর্ব)", "দক্ষিন চরখিজিরপুর"],
        "শাকপুরা ইউনিয়ন": ["পশ্চিম শাকপুরা (উত্তর পশ্চিমাংশ)", "পশ্চিম শাকপুরা (২ নং ওয়ার্ড অংশ)", "পশ্চিম শাকপুরা (দক্ষিণাংশ)", "পশ্চিম শাকপুরা (৪ নং ওয়ার্ড অংশ)", "মধ্যম শাকপুরা (৫ নং ওয়ার্ড অংশ)", "মধ্যম শাকপুরা (৬ নং ওয়ার্ড অংশ)", "মধ্যম শাকপুরা (উত্তরাংশ)", "পূর্ব শাকপুরা", "ঘোষখীল (মধ্যম শাকপুরা পূর্বাংশ)"],
        "সারোয়াতলী ইউনিয়ন": ["পূর্ব সরোয়াতলী ও ছনদন্ডী (পূর্বাংশ)", "পশ্চিম সরোয়াতলী ছনদন্ডী (পশ্চিমাংশ)", "বেংগুরা", "হোরারবাগ", "পশ্চিম খিতাবচর", "পূর্ব খিতাবচর", "ইমামুল্লারচর", "দক্ষিণ সারোয়াতলী", "উত্তর সারোয়াতলী"],
        "পোপাদিয়া ইউনিয়ন": ["সৈয়দপুর", "উত্তর আকুবদন্ডী", "দক্ষিণ আকুবদন্ডী", "পোপাদিয়া (দক্ষিণাংশ)", "পোপাদিয়া (উত্তরাংশ)", "পোপাদিয়া (অংশ) (পশ্চিমাংশ)", "আকালিয়", "পোপাদিয়া", "বিদুগ্রাম (পশ্চিমাংশ)", "বিদুগ্রাম (পূর্বাংশ)"],
        "চরনদ্বীপ ইউনিয়ন": ["চরনদ্বীপ (১ নং ওয়ার্ড অংশ)", "পশ্চিম চরনদ্বীপ", "পাঠানপাড়া (৩ নং ওয়ার্ড অংশ)", "বারৈপাড়া (৪ নং ওয়ার্ড অংশ)", "বারৈপাড়া (৫ নং ওয়ার্ড অংশ)", "বারৈপাড়া (৬ নং ওয়ার্ড অংশ)", "চরনদ্বীপ (৭ নং ওয়ার্ড অংশ)", "চরনদ্বীপ (৮ নং ওয়ার্ড অংশ)", "পূর্ব চরনদ্বীপ"],
        "শ্রীপুর-খরনদ্বীপ ইউনিয়ন": ["পশ্চিম খরনদ্বীপ", "খরনদ্বীপ মাধ্যম", "খরনদ্বীপ", "শ্রীপুর (৪ নং ওয়ার্ড অংশ)", "শ্রীপুর (৫ নং ওয়ার্ড অংশ)", "শ্রীপুর (৬ নং ওয়ার্ড অংশ)", "জৈষ্ট্যপুরা (৭ নং ওয়ার্ড অংশ)", "জৈষ্ট্যপুরা (৮ নং ওয়ার্ড অংশ)", "জৈষ্ট্যপুরা (৯ নং ওয়ার্ড অংশ)"],
        "আমুচিয়া ইউনিয়ন": ["কানুনগোপাড়া", "কানুনগোপাড়া/বগাচড়া", "আমুচিয়া", "ধোরলা (৪ নং ওয়ার্ড অংশ)", "ধোরলা (৫ নং ওয়ার্ড অংশ)", "পূর্ব ধোরলা/কানুনগোপাড়া", "ধোরলা (৭ নং ওয়ার্ড অংশ)", "ধোরলা (৮ নং ওয়ার্ড অংশ)", "পূর্ব ধোরলা"],
        "আহল্লা করলডেংগা ইউনিয়ন": ["উত্তর করলডেংগা", "মধ্যম করলডেংগা", "দক্ষিণ করলডেংগা", "দক্ষিণ উত্তর ভূর্ষি", "উত্তর ভূর্ষি (৫ নং ওয়ার্ড অংশ)", "উত্তর ভূর্ষি (৬ নং ওয়ার্ড অংশ)", "আহল্লা সাধারপাড়া", "আহল্লা শেখ চৌধুরী পাড়া", "আহল্লা শেখ চৌধুরী পাড়া (দক্ষিন)"]
    }
}

# --- ডাটাবেস কানেকশন ---
@st.cache_resource
def init_connection():
    try:
        return create_client(st.secrets["SUPABASE_URL"], st.secrets["SUPABASE_KEY"])
    except:
        st.error("Supabase Secrets পাওয়া যায়নি!")
        st.stop()

supabase = init_connection()

# --- মেইন UI ---
st.title("🗳️ ভোটার তথ্য যাচাইকরণ")

# --- পপ-আপ সেকশন (যদি কোনো ভোটার সিলেক্ট করা থাকে) ---
if st.session_state.selected_voter:
    v = st.session_state.selected_voter
    
    # পপ-আপ এর মতো দেখতে বক্স
    with st.container():
        st.markdown(f"""
        <div class="detail-box">
            <h3 style="color:#0d6efd; margin-top:0;">{v.get('name', 'N/A')}</h3>
            <hr>
        </div>
        """, unsafe_allow_html=True)
        
        # ডাটা দেখানো (Markdown এড়িয়ে st.write ব্যবহার করা হচ্ছে যাতে JS Error না হয়)
        col1, col2 = st.columns(2)
        with col1:
            st.caption("পিতা")
            st.write(v.get('father', '-'))
            st.caption("মাতা")
            st.write(v.get('mother', '-'))
            st.caption("ভোটার নং")
            st.write(v.get('voter_no', '-'))
            st.caption("জন্ম তারিখ")
            st.write(v.get('dob', '-'))
            st.caption("পেশা")
            st.write(v.get('profession', '-'))
            
        with col2:
            st.caption("ঠিকানা")
            st.write(v.get('address', '-'))
            st.caption("ভোটার এলাকা")
            st.write(f"{v.get('voter_area_name')} ({v.get('area_code')})")
            st.caption("ভোটকেন্দ্র")
            st.write(v.get('polling_center', '-'))
            st.caption("সিরিয়াল নং")
            st.write(v.get('serial_no', '-'))

        # বন্ধ করার বাটন
        st.button("❌ বন্ধ করুন", on_click=close_details, type="primary")
        st.divider()

# --- ইনপুট ফর্ম ---
st.subheader("১. এলাকা নির্বাচন")
col_loc1, col_loc2, col_loc3 = st.columns(3)

with col_loc1:
    main_cat = st.selectbox("প্রশাসনিক এলাকা", list(LOCATION_DATA.keys()))

with col_loc2:
    raw_sub_cats = list(LOCATION_DATA[main_cat].keys())
    sub_cat_options = ["নির্বাচন করুন"] + raw_sub_cats
    sub_cat = st.selectbox("ওয়ার্ড বা ইউনিয়ন", sub_cat_options)

with col_loc3:
    if sub_cat == "নির্বাচন করুন":
        selected_area_name = st.selectbox("ভোটার এলাকা", ["আগে ওয়ার্ড/ইউনিয়ন বাছুন"], disabled=True)
    else:
        raw_area_options = LOCATION_DATA[main_cat][sub_cat]
        area_options = ["নির্বাচন করুন"] + raw_area_options
        selected_area_name = st.selectbox("ভোটার এলাকা (গ্রাম)", area_options)

st.subheader("২. ব্যক্তি শনাক্তকরণ")
st.caption("জন্ম তারিখ (বাংলা)")
col_d, col_m, col_y = st.columns(3)
with col_d: day_input = st.selectbox("দিন", DAYS)
with col_m: month_input = st.selectbox("মাস", MONTHS)
with col_y: year_input = st.selectbox("বছর", YEARS)

col_g, col_n, col_p = st.columns(3)
with col_g: gender = st.selectbox("লিঙ্গ (বাধ্যতামূলক)", ["পুরুষ", "মহিলা", "হিজড়া"])
with col_n: name_input = st.text_input("নাম", placeholder="নামের অংশ...")
with col_p: parent_input = st.text_input("পিতা/মাতা", placeholder="নামের অংশ...")

# --- লজিক চেক ---
search_mode = "none"
error_msg = ""
dob_query_bn = None

if day_input != "দিন" and month_input != "মাস" and year_input != "বছর":
    d_val = day_input
    if len(d_val) == 1: d_val = "০" + d_val
    m_val = MONTH_MAP_BN[month_input]
    y_val = year_input
    dob_query_bn = f"{d_val}/{m_val}/{y_val}"

if sub_cat == "নির্বাচন করুন":
    if dob_query_bn and name_input: search_mode = "wide"
    else: error_msg = "⚠️ বিশাল এলাকায় (সিটি/পৌরসভা) খুঁজতে হলে **জন্ম তারিখ** এবং **নাম** দুটিই দেওয়া বাধ্যতামূলক।"
elif selected_area_name == "নির্বাচন করুন":
    if dob_query_bn or name_input or parent_input: search_mode = "mid"
    else: error_msg = "⚠️ পুরো ওয়ার্ড বা ইউনিয়নে খুঁজতে হলে লিঙ্গের সাথে অন্তত নাম, পিতা/মাতা অথবা জন্ম তারিখ দিতে হবে।"
else:
    search_mode = "narrow"

# --- সার্চ বাটন ---
if st.button("অনুসন্ধান করুন"):
    if error_msg:
        st.error(error_msg)
    else:
        with st.spinner("অনুসন্ধান চলছে..."):
            try:
                query = supabase.table("Voter").select("*")
                
                # লোকেশন ফিল্টার
                if search_mode == "wide": pass 
                elif search_mode == "mid":
                    all_areas_in_subcat = LOCATION_DATA[main_cat][sub_cat]
                    query = query.in_("voter_area_name", all_areas_in_subcat)
                elif search_mode == "narrow":
                    query = query.eq("voter_area_name", selected_area_name)
                
                # কমন ফিল্টার
                query = query.eq("gender", gender)
                if dob_query_bn: query = query.ilike("dob", f"%{dob_query_bn}%")
                if name_input: query = query.ilike("name", f"%{name_input}%")
                if parent_input: query = query.or_(f"father.ilike.%{parent_input}%,mother.ilike.%{parent_input}%")

                response = query.limit(100).execute()
                data = response.data

                if not data:
                    st.error(f"❌ কোনো ভোটার পাওয়া যায়নি।")
                else:
                    st.success(f"✅ {len(data)} জন ভোটার পাওয়া গেছে")
                    
                    # লিস্ট দেখানো
                    for voter in data:
                        with st.container():
                            # কার্ড ডিজাইন (HTML/CSS)
                            st.markdown(f"""
                            <div class="voter-card">
                                <div class="voter-name">{voter.get('name', 'নাম নেই')}</div>
                                <div class="voter-sub">পিতা: {voter.get('father', '-')} | ভোটার নং: {voter.get('voter_no', '-')}</div>
                            </div>
                            """, unsafe_allow_html=True)
                            
                            # বিস্তারিত বাটন (Callback দিয়ে State সেট করা হচ্ছে)
                            # এটি পেজ রিফ্রেশ করলেও ডাটা হারাবে না
                            st.button("বিস্তারিত দেখুন", 
                                      key=f"btn_{voter.get('id', voter.get('serial_no'))}",
                                      on_click=view_details,
                                      args=(voter,))
                            
            except Exception as e:
                st.error(f"সার্ভার এরর: {e}")
